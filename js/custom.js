...
// Force layout refresh when clicking filters
...
$('.service-carousel-one').owlCarousel({
    ...
    margin: 15,
    autoplay: true,
    autoplayTimeout: 2500, // Slightly faster for 5 cards
    smartSpeed: 800, // Smoother transition between slides
    autoplayHoverPause: true,
    responsive: {
        0: { items: 1 },
        600: { items: 2 },
        1000: { items: 3 },
        1400: { items: 5 } // This triggers 5 cards on large screens
    }
});
...
$(document).ready(function() {
    ...
    function countUp() {
        ...
        if (bottom_of_screen > top_of_element) {
            ...
            $('.feature-style-one h3').each(function() {
                var $this = $(this);
                // Extract number from text (e.g., "100% Quality" -> 100)
                var fullText = $this.text();
                var number = fullText.match(/\d+/);
                
                if (number) {
                    var targetNum = parseInt(number[0]);
                    var suffix = fullText.replace(number[0], ''); // Keep %, /, etc.
                    
                    $({ countNum: 0 }).animate({ countNum: targetNum }, {
                        duration: 2000,
                        easing: 'swing',
                        step: function() {
                            $this.html('<span class="counter-value">' + Math.floor(this.countNum) + '</span>' + suffix);
                        },
                        complete: function() {
                            $this.html('<span class="counter-value">' + this.countNum + '</span>' + suffix);
                        }
                    });
                }
            });
...
$(document).ready(function() {
    $('.accrodion-title').on('click', function() {
        ...
        if (accrodion.hasClass('active')) {
            accrodion.removeClass('active').find('.accrodion-content').slideUp(300);
        } else {
            // Optional: Close others when opening new one
            $('.accrodion').removeClass('active').find('.accrodion-content').slideUp(300);
            
            accrodion.addClass('active').find('.accrodion-content').slideDown(300);
        }
    });
});
